                ...
                <div class="col-md-6">
                    <div class="card card-primary">
                        <div class="card-header">
                            <h3 class="card-title">Data Relasi Penyakit & Gejala</h3>
                        </div>
                        <form action="<?= base_url('simpan_relasi'); ?>" method="post">
                            <?= csrf_field() ?>
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <select name="pyk_id" class="form-control custom-select <?php if (session('errors.pyk_id')) : ?>is-invalid<?php endif  ?>">
                                        ...
                                        <?php foreach ($kodePenyakit as $kodeP) : ?>
                                            <option value="<?= $kodeP['id']; ?>" <?= (old('pyk_id') == $kodeP['id']) ? 'selected' : ''; ?>><?= $kodeP['kode'] . ' - ' . $kodeP['nama']; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="gjl_id" class="form-control custom-select <?php if (session('errors.gjl_id')) : ?>is-invalid<?php endif  ?>">
                                        ...
                                        <?php foreach ($kodeGejala as $kodeG) : ?>
                                            <option value="<?= $kodeG['id']; ?>" <?= (old('gjl_id') == $kodeG['id']) ? 'selected' : ''; ?>><?= $kodeG['kode'] . ' - ' . $kodeG['nama']; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                    <div class="invalid-feedback">
