                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select name="penyakit_id" class="form-control custom-select <?php if (session('errors.penyakit_id')) : ?>is-invalid<?php endif  ?>">
                                            ...
                                            <?php foreach ($penyakit as $pyk) : ?>
                                                <option value="<?= $pyk['id']; ?>" <?= (old('penyakit_id', $kasusLama['penyakit_id']) == $pyk['id']) ? 'selected' : ''; ?>><?= $pyk['nama']; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                            ...
                                            <?php foreach ($namaGejala as $gejala) : ?>
                                                <tr>
                                                    ...
                                                    <td style="text-align: center;">
                                                        <div class="form-check">
                                                            <input type="checkbox" name="kasusLama[]" class="form-check-input <?php if (session('errors.kasusLama')) : ?>is-invalid<?php endif; ?>" value="<?= $gejala['id']; ?>" <?= (in_array($gejala['id'], $gejalaLama)) ? 'checked' : ''; ?>>
                                                            <br>
                                                            <div class="invalid-feedback">
                                                                <?= session('errors.kasusLama') ?>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="form-group row justify-content-md-center">
                                                            <div class="col-md-8">
                                                                <select name="bobot[]" class="form-control custom-select bobotSelect" data-gejala="<?= $gejala['id']; ?>">
